...
Architectural brief for every Claude Code session in this repo. Read `start_prompt.md` for full product/roadmap context, and `CONTEXT.md` for current working state, first if this is a new session. This file is synced with `AGENTS.md` — update both together.
...
└── .input/               # gitignored — scratch for data Arjun manually pastes (ESPN pages the API can't reach)
```
...
- Current work and session state: `CONTEXT.md` — **update it at the end of every session** (current phase + a dated session-notes entry), same pattern as bookshelf
